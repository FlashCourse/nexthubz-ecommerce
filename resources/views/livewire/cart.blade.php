            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($cartItems as $item)
                    <div class="bg-white overflow-hidden rounded-lg shadow-md relative">
                        <button
                            class="absolute top-2 right-2 bg-green-500 text-white px-2 py-1 rounded-full hover:bg-green-600 transition duration-300"
                            wire:click="removeFromCart('{{ $item['id'] }}')">
                            Remove
                        </button>
                        <img src="{{ $item['image'] }}" alt="{{ $item['name'] }}"
                            class="w-full h-40 object-cover mb-4 rounded-t-md">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <input type="number" min="1"
                                        wire:model="cartItems.{{ $item['id'] }}.quantity"
                                        class="w-12 px-2 py-1 border rounded-md text-center">
                                    <span class="ml-2 text-gray-500">Qty</span>
                                </div>
                                <p class="text-indigo-500 font-bold"><span
                                        class="text-lg font-extrabold">&#2547;</span>{{ $item['price'] }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
