    ...
    <div class="mx-auto max-w-7xl">
        @if (session()->has('message'))
            <x-modal id="myModal" maxWidth="lg">
                <div class="p-4 bg-green-500 text-white text-center"> {{ session('message') }}</div>
            </x-modal>
        @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="flex items-center p-4 bg-gray-100 rounded-md">
                        <input type="radio" wire:model="paymentMethod" id="cash" name="paymentMethod"
                            value="cash" class="w-4 h-4 text-green-500 focus:ring-green-500" requigreen>
                        <label for="cash" class="ml-4 text-gray-600 hover:cursor-pointer flex items-center">
                            <i class="fas fa-money-bill-wave text-2xl text-green-500 mr-2"></i>
            ...
            <div class="p-6  rounded-lg border">
                        ...
                        <div class="grid gap-4">
                            @foreach ($cart as $item)
                                <div class="p-4 bg-white rounded-lg flex items-center justify-between">
                                    ...
                                    <span
                                        class="font-semibold text-green-500">&#2547;{{ $item['price'] * $item['quantity'] }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <!-- Replace with dynamic data -->
                        <p class="text-lg font-bold text-green-500">&#2547;{{ number_format($total, 2) }}</p>
                    </div>
                </div>
        ...
        {{ $stockAvailable ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-400 cursor-not-allowed' }}"
